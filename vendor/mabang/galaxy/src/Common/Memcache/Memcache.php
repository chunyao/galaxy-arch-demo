...
namespace Mabang\Galaxy\Common\Memcache;

use Mabang\Galaxy\Common\Memcache\Config;
use Mabang\Galaxy\Common\Memcache\Exception\ConnectException;
use Mabang\Galaxy\Common\Memcache\Exception\MemcacheException;
use Mabang\Galaxy\Common\Memcache\Opcode;
use Mabang\Galaxy\Common\Memcache\Package;
use Mabang\Galaxy\Common\Memcache\Status;
use Swoole\Coroutine\Client as CoroutineClient;

/**
 * 协程Memcache客户端
 * Class Memcache
 * @package Galaxy\Common\Memcache
 */
class Memcache
{
    private $config;
    ...
    private function checkStatus(Package $package, $except = [])
    {
        ...
        if (!in_array($status, $except) && $status !== Status::STAT_NO_ERROR) {
            ...
            $exception = new MemcacheException("Memcache Error: ". $package->getStatus());
